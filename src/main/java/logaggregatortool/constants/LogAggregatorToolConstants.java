package logaggregatortoolconstants;

/*
 * This class defines constants used in the Log Aggregator Tool.
 * It contains various string meassages used for logging and validation.*/
public class LogAggregatorToolConstants {
    /**
     * Message displayed when the specified folder is empty.
     */
    public static final String ERROR_OCCURED = "Error occurred while reading file:";
    public static final String FOLDER_EMPTY = "Folder is Empty.";
    public static final String INVALID_FILES = "Count of invalid files:";
    public static final String INVALID_FOLDER = "Invalid_Folderpath .please provide a valid folderpath.";
    public static final String PROCESSING_MESSAGE = "Processing";
    public static final String SORTED_LOGS = "Sorted Log Entries:";
    public static final String TIMESTAMP_NOT_FOUND = "Timestamp not found in log:";
    public static final String TOTAL_FILES = "Total count of files:";
    public static final String VALID_FILES = "Count of valid Files:";
}